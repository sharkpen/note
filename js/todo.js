...
function queryTasksLengthByCateId(id) {
    ...
    if (cate.child.length !== 0) {
        for (var i = 0; i < cate.child.length; i++) {
            var childCate = queryChildCatesById(cate.child[i].id);
 ...
 * @return {Array}    任务对象数组
 */
 //.concat用于连接前后的字符串,并按顺序连接在resultArr的尾部
...
function initModal(){
	...
	for (var i = 0; i < cate.length; i++) {
		seleccontent = '<option value="' + cate[i].id + '">' + cate[i].name + '</option>';
	};
	$("#modal-select").innerHTML = selectContent;
	$("#newCateName").innerHTML = "";
}
...
//设置高亮，为对象添加类名active
function setHightLight(element){
	cleanAllActive();
